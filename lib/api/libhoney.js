...
const beelineOptions = new Set([
  "serviceName",
  "sampleRate",
  "samplerHook",
  "presendHook",
  "baseFields",
  "reportInstrumentations",
  "appVersion",
]);

const getLibHoneyOptions = opts => {
  let filteredOptions = {};
  Object.keys(opts).forEach(optionKey => {
    if (!beelineOptions.has(optionKey)) {
...
module.exports = class LibhoneyEventAPI {
  constructor(opts) {
    ...
    this.reportInstrumentations = true;
    if (typeof opts.reportInstrumentations === "boolean") {
      this.reportInstrumentations = opts.reportInstrumentations;
    }
    ...
    this.honey.add({
      ...
      [schema.APP_VERSION]: this.appVersion,
    });
    if (opts.baseFields) {
      debug(`adding base fields %j`, opts.baseFields);
      this.honey.add(opts.baseFields);
    }
  }
  ...
  finishSpan(span, rollup) {
    ...
    tracker.runWithoutTracking(() => {
      ...
      if (this.reportInstrumentations) {
